<script>
  import { clockTime, preferences } from '../stores/app-store.js';
  ...
  $: showSeconds = $preferences.panel.showSeconds !== false;
  $: seconds = timeParts[2] ?? '00';
  $: dateLine = $activeFormatters.longDate.format(now);
...
<section class="clock-block" aria-labelledby="clock-title">
  ...
  <div class="clock-face">
    ...
    {#if showSeconds}<span class="clock-seconds" aria-hidden="true">{seconds}</span>{/if}
  </div>
