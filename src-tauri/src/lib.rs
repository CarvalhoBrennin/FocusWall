...
fn enforce_safe_state_file_metadata(path: &Path) -> Result<(), String> {
    let metadata = fs::symlink_metadata(path).map_err(|error| error.to_string())?;
    if metadata.file_type().is_symlink() {
        return Err("Arquivo de estado não pode ser link simbólico".to_string());
    }
    if metadata.len() as usize > MAX_STATE_FILE_BYTES {
        return Err("Arquivo de estado excede limite seguro de tamanho".to_string());
    }
